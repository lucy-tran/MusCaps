...
class COCO:
    ...
    def getAnnIds(self, audioIds=[], catIds=[], areaRng=[], iscrowd=None):
        ...
        if len(audioIds) == len(catIds) == len(areaRng) == 0:
            ...
            if not len(audioIds) == 0:
                anns = sum([
                    self.audioToAnns[audioId]
                    for audioId in audioIds if audioId in self.audioToAnns
                ], [])
            else:
                anns = self.dataset['annotations']
    ...
    def loadRes(self, resFile):
        ...
        """
        res = COCO()
        res.dataset['audio samples'] = [
            audio for audio in self.dataset['audio samples']
        ]
        ...
        if 'caption' in anns[0]:
            audioIds = set([
                audio['id'] for audio in res.dataset['audio samples']
            ]) & set([ann['audio_id'] for ann in anns])
            res.dataset['audio samples'] = [
                audio for audio in res.dataset['audio samples']
                if audio['id'] in audioIds
            ]
